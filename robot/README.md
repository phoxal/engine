...
`phoxal-robot` owns the `robot.yaml` source schema and the shared
deterministic foundation contracts:

- `phoxal_robot::Robot` is the version dispatcher for `robot.yaml`.
- `phoxal_robot::v1::Robot` describes identity, runtime resolution intent, simulation intent,
...
- All v1 wire types live under `phoxal_robot::v1`; import
  `phoxal_robot::RobotV1` as a crate-root alias when a direct v1 type is
  more convenient.
