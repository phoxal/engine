...
impl Robot {
    pub(crate) fn validate_component_structure(
        ...
        for (component_id, component) in &self.components {
            if !phoxal_component::v1::is_valid_token(component_id) {
            ...
            if !phoxal_component::v1::is_valid_token(&component.component) {
                validation_errors.push(ValidationError::InvalidToken {
            ...
            for (capability_key, parameters) in &component.parameters {
                if !phoxal_component::v1::is_valid_token(capability_key) {
                    validation_errors.push(ValidationError::InvalidToken {
            ...
            for capability_id in component.roles.keys() {
                if !phoxal_component::v1::is_valid_token(capability_id) {
                    validation_errors.push(ValidationError::InvalidToken {
...
fn validate_capability_ref(
    ...
    if !phoxal_component::v1::is_valid_token(&capability_ref.component_id)
        || !phoxal_component::v1::is_valid_token(&capability_ref.capability_id)
    {
