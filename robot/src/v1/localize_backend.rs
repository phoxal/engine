...
pub fn resolve_localize_backend(
    model: &Robot,
    components: &BTreeMap<String, phoxal_component::v1::Component>,
...
mod tests {
    ...
    fn fixture_model_and_components() -> (Robot, BTreeMap<String, phoxal_component::v1::Component>)
    {
    ...
    fn fixture_bundle_root() -> PathBuf {
        ...
        // robot sits at the framework workspace root after the
        // engine/runtimes merge — one level below the root.
        let workspace_root = match manifest_dir.parent() {
            Some(path) => path,
            None => panic!(
                "robot CARGO_MANIFEST_DIR must live one level below the workspace root: {}",
    ...
    fn add_gnss_localization_component(
        model: &mut Robot,
        components: &mut BTreeMap<String, phoxal_component::v1::Component>,
    ) {
        components.insert(
            "zed_f9p".to_string(),
