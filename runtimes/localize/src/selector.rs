...
use phoxal_utils_robot::v1::{ResolvedLocalizeBackend, resolve_localize_backend};
...
fn camera_optical_to_base(
    ...
    let transform = phoxal_utils_spatial::sensor::resolve_capability_link_pose_in_frame(
        &robot.model,
        &robot.components,
        structure,
        camera,
...
fn imu_to_camera_optical(
    ...
    let camera_link_in_base = phoxal_utils_spatial::sensor::resolve_capability_link_pose_in_frame(
        &robot.model,
        &robot.components,
        structure,
        camera,
    ...
    let imu_link_in_base = phoxal_utils_spatial::sensor::resolve_capability_link_pose_in_frame(
        &robot.model,
        &robot.components,
        structure,
        imu,
...
mod tests {
    ...
    fn fixture_bundle_root() -> PathBuf {
        ...
        let workspace_root = match manifest_dir.parent().and_then(|path| path.parent()) {
            Some(path) => path,
            None => panic!(
                "runtimes/localize CARGO_MANIFEST_DIR must live two levels below the workspace root: {}",
