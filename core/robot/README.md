...
`phoxal-core-robot` owns the `robot.yaml` source schema and the shared
deterministic foundation contracts:

- `phoxal_core_robot::Robot` is the version dispatcher for `robot.yaml`.
- `phoxal_core_robot::v1::Robot` describes identity, runtime resolution intent, simulation intent,
...
- All v1 wire types live under `phoxal_core_robot::v1`; import
  `phoxal_core_robot::RobotV1` as a crate-root alias when a direct v1 type is
  more convenient.
...
```rust
use phoxal_core_robot::v1::Robot;
