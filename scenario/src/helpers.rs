...
pub fn fixture_robot(fixture_bundle: &str) -> Result<staged::Robot> {
    ...
    let model = phoxal_robot::v1::Robot::read_from_dir(&bundle_path)
    ...
    let components = model
        ...
        .map(|component_type| {
            ...
            let component = phoxal_component::Component::read_from_dir(&component_dir)
                .with_context(|| {
                    format!(
...
///
/// Runtime crates live at `runtimes/<name>/runtime/`, so walking three parents up
/// reaches the workspace root. The debug assertion catches layout drift.
pub fn workspace_root() -> Result<PathBuf> {
    ...
    let Some(workspace_root) = manifest_dir
        ...
        bail!(
            "runtime CARGO_MANIFEST_DIR must live three levels below the workspace root: {}",
