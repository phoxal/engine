...
impl ScenarioEnvironment {
    ...
    fn from_vars(
        ...
        let robot_router_endpoint =
            var(phoxal_engine::ENV_ROBOT_ROUTER_ENDPOINT).with_context(|| {
                format!(
                    "{} must be set for scenario context",
                    phoxal_engine::ENV_ROBOT_ROUTER_ENDPOINT
                )
            })?;
        let robot_id = var(phoxal_engine::ENV_ROBOT_ID).with_context(|| {
            format!(
                "{} must be set for scenario context",
                phoxal_engine::ENV_ROBOT_ID
            )
        })?;
        let robot_namespace = var(phoxal_engine::ENV_ROBOT_NAMESPACE)
    ...
    pub fn new(
        ...
        let environment = Self {
            ...
            robot_namespace: trim_required(
                ...
                phoxal_engine::ENV_ROBOT_NAMESPACE,
            )?,
            robot_id: trim_required(robot_id.into(), phoxal_engine::ENV_ROBOT_ID)?,
        };
        Ok(environment)
    }

    pub async fn connect(self, wallclock_timeout: Duration) -> Result<ScenarioContext> {
        let bus = phoxal_bus::builder::Builder::new(self.robot_router_endpoint.clone())
...
impl ScenarioContext {
    ...
    pub async fn reset_simulation(&self) -> Result<u64> {
        let retry =
            phoxal_bus::query::Retry::new(3).with_initial_backoff(Duration::from_millis(50));
...
mod tests {
    ...
    fn environment_uses_existing_runtime_env_names() -> anyhow::Result<()> {
        let vars = BTreeMap::from([
            (
                phoxal_engine::ENV_ROBOT_ROUTER_ENDPOINT.to_string(),
                "tcp/127.0.0.1:7447".to_string(),
            ),
            (
                phoxal_engine::ENV_ROBOT_NAMESPACE.to_string(),
                "dev".to_string(),
            ),
            (
                phoxal_engine::ENV_ROBOT_ID.to_string(),
                "robot-a".to_string(),
            ),
        ]);
