...
pub const GOAL_INPUT_TOPIC: &str = phoxal_api_mission::v1::GOAL_TOPIC;
...
phoxal_infra_bus::pubsub_leaf!(path, PATH_TOPIC, Path);
phoxal_infra_bus::pubsub_leaf!(state, STATE_TOPIC, State);

pub mod debug {
    use super::*;

    phoxal_infra_bus::pubsub_leaf!(search_graph, DEBUG_SEARCH_GRAPH_TOPIC, SearchGraph);
    phoxal_infra_bus::pubsub_leaf!(cost_layers, DEBUG_COST_LAYERS_TOPIC, CostLayers);
    phoxal_infra_bus::pubsub_leaf!(rejected_paths, DEBUG_REJECTED_PATHS_TOPIC, RejectedPaths);
    phoxal_infra_bus::pubsub_leaf!(revision_inputs, DEBUG_REVISION_INPUTS_TOPIC, RevisionInputs);
}
