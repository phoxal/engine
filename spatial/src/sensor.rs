...
use phoxal_component::v1::{CapabilityRef, Component as SourceComponent};
use phoxal_robot::v1::Robot;
...
mod tests {
    ...
    use phoxal_component::v1::capability::{
    ...
    -> Result<()> {
        ...
        let model = phoxal_robot::v1::Robot::read_from_dir(&bundle_root)?;
    ...
    fn source_components(
        ...
        model
            ...
            .into_iter()
            .map(|component_type| {
                let component = phoxal_component::Component::read_from_dir(
    ...
    fn workspace_root() -> PathBuf {
        ...
        // spatial sits at the framework workspace root after the
        // engine/runtimes merge — one level below the root.
        let workspace_root = match manifest_dir.parent() {
            Some(path) => path,
            None => panic!(
                "spatial CARGO_MANIFEST_DIR must live one level below workspace root: {}",
